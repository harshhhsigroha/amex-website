/**
 * ============================================================================
 * ClientEndUserManager
 * ============================================================================
 * Used in: ClientDashboard (client-facing)
 * Purpose: Allows a client admin (e.g. Tony) to manage their own portal
 *          end-users — create logins, edit names, change passwords, remove
 *          access, and toggle per-client portal permissions.
 *
 * Key behaviours:
 *   - Fetches the logged-in client's ID from `client_users`
 *   - Lists portal users linked to that client from `portal_users` + `profiles`
 *   - Creates new end-users via the `create-client-user` edge function
 *   - Edits/passwords via the `update-client-password` edge function
 *   - Permissions managed via <PortalPermissionsManager> (per-client toggle)
 * ============================================================================
 */
...
import {
  UserPlus, Trash2, Users, ExternalLink, Loader2, Copy,
  Edit2, Key, Eye, EyeOff, XCircle, ShieldCheck,
} from 'lucide-react';
...
// ── Helper: call the update-client-password edge function ────────────────────
async function callUpdateFn(body: Record<string, unknown>) {
...
export function ClientEndUserManager() {
  ...
  // ── Data: resolve current user's client ID ──────────────────────────────
  ...
  // ── Data: portal users for this client ──────────────────────────────────
  ...
  const createUser = useMutation({
    ...
    onSuccess: () => {
      toast.success('End user created', {
        description: `${form.email} can now log in at /auth/portal`,
      });
      setShowAdd(false);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-muted-foreground">
            {portalUsers.length} end user{portalUsers.length !== 1 ? 's' : ''}
          </span>
        </div>
        ...
        <Card className="border-border/60">
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">
              This user will be able to log in via the End User portal at{' '}
              <code className="font-mono">/auth/portal</code>.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="flex gap-2 pt-1">
              <Button
                ...
                disabled={createUser.isPending || !form.email || !form.password || !form.fullName || !isPasswordStrong(form.password)}
              >
                {createUser.isPending && <Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" />}
                Create User
              </Button>
      ...
      <div className="space-y-2">
            ...
            <CardContent className="py-12 flex flex-col items-center gap-3 text-center">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Add your first end user to give them portal access
                </p>
              </div>
        ...
        {portalUsers.map((u) => (
          <div
            key={u.id}
            className="flex items-center justify-between px-4 py-3 rounded-lg border border-border/60 bg-card hover:bg-muted/20 transition-colors group"
          >
            <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1">
              ...
              <p className="text-xs text-muted-foreground hidden sm:block mr-2">
        ...
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Edit2 className="h-4 w-4" />Edit User Info
            </DialogTitle>
      ...
      <Dialog open={!!pwUser} onOpenChange={() => setPwUser(null)}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3 py-2">
            <div className="p-3 bg-muted/50 border border-border/60 rounded-lg text-xs text-muted-foreground">
      ...
      {/* ── Dialog: Portal Permissions ───────────────────────────────────── */}
      <Dialog open={!!permsUser} onOpenChange={() => setPermsUser(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <ShieldCheck className="h-4 w-4" />Portal Permissions
            </DialogTitle>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will revoke their portal access. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => deleteId && removeUser.mutate(deleteId)}
            >
              Remove
            </AlertDialogAction>
          </AlertDialogFooter>
